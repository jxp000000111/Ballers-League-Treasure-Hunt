...
def ensure_hunt_timing_row(team_name: str):
    existing = get_hunt_timing(team_name)
    if existing:
        return existing

    supabase.table("th_hunt_timing").upsert(
        {
            "team_name": team_name
        },
    ...
    return get_hunt_timing(team_name)


def mark_hunt_started(team_name: str):
    row = ensure_hunt_timing_row(team_name)
    if row and row.get("hunt_started_at"):
        return

    now_ts = datetime.now(timezone.utc).isoformat()

    supabase.table("th_hunt_timing").update({
        "hunt_started_at": now_ts
    }).eq("team_name", team_name).execute()
...
def mark_tier_completed(team_name: str, tier: int):
    now_ts = datetime.now(timezone.utc).isoformat()
    field_name = f"tier_{tier}_completed_at"
...
timing_summary_rows = get_timing_summary()
timing_map = {row["team_name"]: row for row in timing_summary_rows}
...
if not st.session_state.show_clue:
    ...
    st.markdown('<div class="muted-text">Tap your team card below to enter.</div>', unsafe_allow_html=True)
...
ensure_hunt_timing_row(selected_team)
timing_row = get_hunt_timing(selected_team)
hunt_started = bool(timing_row and timing_row.get("hunt_started_at"))
...
if not hunt_started:
    ...
    if image_url and isinstance(image_url, str) and image_url.strip().lower() not in ["0", "none", "null", ""]:
        try:
            ...
            st.warning("Start question image could not be loaded.")

    st.markdown(f"### {start_gate['question_text']}")

    start_answer = st.text_input("Type your answer", key=f"start_answer_{selected_team}")

    if st.button("Start Hunt", use_container_width=True):
        if not start_answer or not start_answer.strip():
            st.error("Please answer the start question.")
        else:
            user_answer = normalize_answer(start_answer)
            correct_answer = normalize_answer(start_gate["correct_answer"])

            if user_answer == correct_answer:
                mark_hunt_started(selected_team)
                st.success("Correct answer. Your official timer has started.")
                st.rerun()
            else:
...
if quiz and quiz_unlocked and not quiz_completed:
    ...
    st.markdown('<div class="muted-text">Answer correctly to complete this tier and unlock the next clue.</div>', unsafe_allow_html=True)
    ...
    if st.button("Submit Quiz Answer", use_container_width=True):
        if not answer or not answer.strip():
            ...
            if user_answer == correct_answer:
                ...
                mark_tier_completed(selected_team, current_tier)
